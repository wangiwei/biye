...
# --------------- 松弛模型 -----------
def enhance_ubound(q, h1, h2, c, f, level, facility, demand, x):
	"""
	...
	# 容量约束 -> 将要被松弛
	mdl.add_constraints(
	    (mdl.sum(mdl.z[i, j, k] * h1[i] for i in mdl.I for k in mdl.K) <= c[j] * mdl.x[j]) for j in
	    mdl.J)
	mdl.add_constraints(
	    (mdl.sum(mdl.z[i, j, k] * h2[i] for i in mdl.I for k in mdl.K) <= c[j] * mdl.x[j]) for j in
	    mdl.J)
